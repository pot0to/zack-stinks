...
If you see "Python was not found; run without arguments to install from the Microsoft Store", Windows is intercepting the `python` command. To fix this, go to Settings > Apps > Advanced app settings > App execution aliases, then turn off the toggles for "python.exe" and "python3.exe". After disabling these, close and reopen Git Bash (or your terminal) for the changes to take effect.
